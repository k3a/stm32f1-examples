...
static int 	mbedtls_entropy_func_fake (void *data, unsigned char *output, size_t len) {
	memset(output, 1, len);
	return 0;
}

static volatile int dbg(const char* fmt, ...) {
	puts(fmt);
	asm("nop");
}
...
int main( int argc, char *argv[] )
{
    ...
    //k3ambedtls_entropy_context entropy;
    //k3ambedtls_ctr_drbg_context ctr_drbg;
    ...
    //k3ambedtls_ctr_drbg_init( &ctr_drbg );
    ...
    if( ( ret = mbedtls_ctr_drbg_seed( &ctr_drbg, mbedtls_entropy_func_fake, /*&entropy*/NULL,
    ...
    ret = mbedtls_ecdh_gen_public( &ctx_cli.grp, &ctx_cli.d, &ctx_cli.Q,
                                   /*mbedtls_ctr_drbg_random, &ctr_drbg*/mbedtls_entropy_func_fake,NULL );
    if( ret != 0 )
    {
